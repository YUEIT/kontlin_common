...
object SslHttpsUtils {
     ...
     * https单向认证
     * 用含有服务端公钥的证书校验服务端证书
     * @param cerResId 证书公钥资源id
     */
    fun getSslSocketFactory(vararg cerResId: Int): SSLParams? {
        val certificates = arrayOfNulls<InputStream>(cerResId.size)
        for (i in cerResId.indices) {
            certificates[i] = getContext().resources.openRawResource(cerResId[i])
        }
        return getSslSocketFactoryBase(null, null, null, *certificates)
    }
